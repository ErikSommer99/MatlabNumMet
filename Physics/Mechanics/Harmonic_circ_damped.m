...
% Resistance
R = [0.00 100.00]; % Compare without and with resistance 
...
% Without resistance
[T1,F1] = ode45(@(t,f_array)Harmonicfuncirc(t,f_array,C,L,R(1),epsilon_m,omega_drive),...
    t_int,InitCond,options);

% With resistance
[T2,F2] = ode45(@(t,f_array)Harmonicfuncirc(t,f_array,C,L,R(2),epsilon_m,omega_drive),...
    t_int,InitCond,options);
...
plot(T1,F1(:,1),'-b','LineWidth',2.0), % Without resistance
...
plot(T2,F2(:,1),'--r','LineWidth',2.0) % With resistance
...
plot(T1,F1(:,2),'-b','LineWidth',2.0), % Without resistance
xlabel('$t$ (s)','FontSize',14,'VerticalAlignment','Top','Interpreter','latex'),
...
plot(T2,F2(:,2),'--r','LineWidth',2.0) % With resistance
...
Q_m = epsilon_m/(L*sqrt(4*alpha^2*omega_drive^2+(omega_drive^2-omega_0^2)^2));
